    ...
    if (isset($amnu) && sizeof($amnu) > 0) {
        ...
        foreach ($amnu as $k1 => $v1) {
            ...
            if (isset($v1['type']) && $v1['type'] == 'myshop' && isset($v1['version']) && $v1['version'] == 1) {
                ...
                if (isset($v1['datain_name_file']) && file_exists($_SERVER['DOCUMENT_ROOT'] . DS . '9.site' 
                        . DS . $now['folder'] . DS . 'download' . DS . 'datain' . DS . $v1['datain_name_file'])) {
                    ...
                    $e2 = \Nyos\mod\MyShopAdmin::loadDataFileForShop( 
                            $db
                            , $e['data']['id']
                            , $_SERVER['DOCUMENT_ROOT'] . DS . '9.site' . DS . $now['folder'] . DS . 'download' . DS . 'datain' . DS . $v1['datain_name_file']
                            );
                    \Nyos\mod\myshop::deleteCash($db, $e['data']['id']);
                    // echo $e2;
                    // $e3 = json_decode($e2)
                ...
                die(' обработка дата файла #' . __LINE__);
            }
        }
        die(' обработка дата файла #' . __LINE__);
    }
