...
// $_REQUEST['action'] => check_item_in_cart

if (isset($_GET['action']) && $_GET['action'] == 'check_item_in_cart') {
    
    if (isset($_GET['action']) && $_GET['action'] == 'check_item_in_cart' && isset($_GET['item']) && is_numeric($_GET['item'])) {

        $it = $_GET['item'] . ( isset($_GET['price_opt']) && is_numeric($_GET['price_opt']) ? '-' . $_GET['price_opt'] : '' );

        // echo '111-'.$it;

        if (
                isset($_SESSION['on_cart'][$it]) && isset($_SESSION['on_cart'][$it]['kolvo']) && $_SESSION['on_cart'][$it]['kolvo'] > 0
        ) {
            f\end2('есть', true);
        }

        f\end2('нет таких', false);
    }

    f\end2('нет таких', false);
}

// f\end2('#'.__LINE__, false);
...
if (isset($_REQUEST['send_order'])) {
    ...
    */
    ...
    $list = \Nyos\mod\myshop::getItems($db);
    // f\pa($list);
    ...
    $info = '<blockquote style="border;left: 3px solid gray; padding-left: 15px; color: black; line-height:30px;" >'
            . 'ФИО: ' . addslashes($_REQUEST['fio']) . '<br/>'
    ...
    $sum = 0;
    foreach ($_REQUEST['kolvo'] as $k2 => $v) {
        ...
        if ($v > 0 && isset($_REQUEST['item_select'][$k2]) && isset(Nyos\mod\myshop::$items[$k])) {

            if( isset($price_opt) && is_numeric($price_opt) && isset(Nyos\mod\myshop::$items[$k]['options'][$price_opt]) ){
            $price = Nyos\mod\myshop::$items[$k]['options'][$price_opt];
            }else{
            ...
            $info .= '<tr>'
                    . '<td>'
                    . '<span class="w">#' . Nyos\mod\myshop::$items[$k]['id'] . '</span> <b>' . Nyos\mod\myshop::$items[$k]['name'] . '</b>'
                    
                    . (!isset(Nyos\mod\myshop::$items[$k]['opis']{150}) ? '<br/><small>' . Nyos\mod\myshop::$items[$k]['opis'] . '</small>' : '' );


                            $t = Nyos\mod\myshop::getItemOptionsData($db, $k2 );
                            //f\pa($t);
                            
                            
                            if( isset($t['status']) && $t['status'] == 'ok' && isset( $t['data'] ) && is_array( $t['data'] ) ){
                                foreach( $t['data'] as $k4 => $v4 ){
                                    $info .= '<br/>'.$v4['name'].': '.$v4['val'];
                                }
                            }
            ...
            $info .=  '</td>'
                    ...
                    . '<td class="r">' . number_format( ceil( $v * $price ), 0, '', '`') . '</td>'
                            
                    . '</tr>';
                    
            $sum += $v * $price;
            
            if (isset($_SESSION['on_cart'][$k2])) 
                unset($_SESSION['on_cart'][$k2]);
    ...
    Nyos\mod\mailpost::sendNow( $db, 
        'support@uralweb.info', 
        'support@uralweb.info' . ( isset(Nyos\mod\myshop::$shop['dop']['email_to_order']{5}) ? ',' . Nyos\mod\myshop::$shop['dop']['email_to_order'] : '' ), 
        'Новый заказ в интернет магазине ' . ( isset($cfg['name_shop']) ? $cfg['name_shop'] : $_SERVER['HTTP_HOST'] ), 
        'nexit_myshop', 
        array('text' => $info));
...
elseif (isset($_REQUEST['type']) && $_REQUEST['type'] == 'add_to_cart' && isset($_REQUEST['id'])) {
    ...
    $new = array(
        'id' => $_REQUEST['id']
        , 'kolvo' => 1
        , 'options' => serialize($_REQUEST['select_option'])
        , 'price_opt' => ( ( isset($_REQUEST['price_opt']{0}) && is_numeric($_REQUEST['price_opt']) ) ? $_REQUEST['price_opt'] : '' )
        , 'price' => ( ( isset($_REQUEST['price']{0}) && is_numeric($_REQUEST['price']) ) ? $_REQUEST['price'] : '' )
    );

    $_SESSION['on_cart'][$_REQUEST['id'] . ( ( isset($_REQUEST['price_opt']{0}) && is_numeric($_REQUEST['price_opt']) ) ? '-' . $_REQUEST['price_opt'] : '' )] = $new;
...
elseif (isset($_REQUEST['type']) && $_REQUEST['type'] == 'get_nadpis_for_cart') {
    ...
    if (isset($_REQUEST['active_items']{0})) {
        ...
        foreach ($_SESSION['on_cart'] as $k => $v) {
            ...
            if (isset($active[$v['id']]) && isset($v['kolvo']{0}) && isset($v['price']{0}))
                $sum += ceil($v['kolvo'] * $v['price']);
        }
    }else {
